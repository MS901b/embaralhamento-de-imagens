...
var parte_3 = new(Class.create({
    ...
    set_inicial: function() {
        ...
        console.log(this.permuta.resposta)

        $('SalvaLocal').Salva(nomeSoft, 'valor_inicial_a2p3', this.permuta.resposta);
...
var parte_4 = new(Class.create({
    ...
    salva: function() {
        $('SalvaLocal').Salva(nomeSoft, 'a2_p4_q' + this.selecionada, this.permuta.resposta);
    },
    ...
    ajusta: function() {
        var resp = JSON.parse($('SalvaLocal').Pega(nomeSoft, 'a2_p4_q' + this.selecionada));
...
function corrige_q_6_a(valor) {
    var resp = ordenaCiclo(montaCiclo(JSON.parse($('SalvaLocal').Pega(nomeSoft, 'a2_p4_q1'))));
    return [resp.length == 3 && resp[0].length == 2 && resp[1].length == 3 && resp[2].length == 4];
}

function corrige_q_7_a(valor) {
    var resp = ordenaCiclo(montaCiclo(JSON.parse($('SalvaLocal').Pega(nomeSoft, 'a2_p4_q2'))));
    return [resp.length == 4 && resp[0].length == 1 && resp[1].length == 2 && resp[2].length == 2 && resp[3].length == 4];
}

function corrige_q_8_a(valor) {
    tam = montaCiclo(JSON.parse($('SalvaLocal').Pega(nomeSoft, 'a2_p4_q3')))
    if (tam == false) {
        return false;
    } else {
        var resp = montaCiclo(JSON.parse($('SalvaLocal').Pega(nomeSoft, 'a2_p4_q3')));
